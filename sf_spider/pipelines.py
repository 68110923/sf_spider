...
class UpdateOrCreatePostgreSQLPipeline(UniversalPostgreSQLPipeline):
    """
    扩展UniversalPostgreSQLPipeline，添加类似Django的update_or_create功能
    支持根据唯一约束条件查找记录，存在则更新，不存在则创建
    
    可以直接在Scrapy的ITEM_PIPELINES中配置，通过yield item自动处理
    """
    
    def process_item(self, item, spider):
        """
        重写process_item方法，使其自动使用update_or_create功能
        当爬虫yield item时，Scrapy会自动调用这个方法
        
        参数:
            item: 要处理的Scrapy Item对象
            spider: 当前运行的爬虫实例
        
        返回:
            item: 处理后的Item对象
        """
        # 直接调用update_or_create_item方法处理item
        try:
            created, processed_item = self.update_or_create_item(item, spider)
            if created:
                spider.logger.debug(f"新记录已创建: {item.get('tracking_number', 'N/A')}")
            else:
                spider.logger.debug(f"现有记录已更新: {item.get('tracking_number', 'N/A')}")
        except Exception as e:
            spider.logger.error(f"处理Item失败: {str(e)}")
            raise DropItem(f"update_or_create处理失败: {str(e)}")
        
        return item
    
    def update_or_create_item(self, item, spider):
        """
        根据唯一约束条件查找并更新或创建记录
        ...
        返回:
            tuple: (created, item_dict) - created为True表示新创建，False表示更新
                  item_dict为处理后的Item数据字典
        """
        # 检查Item是否实现了必要的数据库配置接口
        if not self._is_valid_database_item(item):
            spider.logger.debug("跳过未实现数据库配置接口的Item")
        ...
        # 提取唯一约束字段
        unique_fields = self._get_unique_constraint_fields(item)
        if not unique_fields:
            spider.logger.warning(f"表 {table_name} 没有配置唯一约束，使用默认批量插入")
            self._cache_batch_data(table_name, item_dict)
            return True, item_dict
        
        # 执行update_or_create操作
        created = self._perform_update_or_create(table_name, item_dict, unique_fields, spider)
        return created, item_dict
    
    def _get_unique_constraint_fields(self, item):
        """
        从Item的UNIQUE_CONSTRAINTS中提取唯一约束字段列表
        优先使用第一个联合唯一约束
        """
        if not item.UNIQUE_CONSTRAINTS:
            return []
        
        # 获取第一个约束
        first_constraint = item.UNIQUE_CONSTRAINTS[0]
        
        if isinstance(first_constraint, list):
            # 直接是字段列表
            return first_constraint
        elif isinstance(first_constraint, dict):
            # 字典格式，包含fields键
            return first_constraint.get("fields", [])
        
        return []
    
    def _perform_update_or_create(self, table_name, item_dict, unique_fields, spider):
        """
        执行update_or_create的核心逻辑
        
        参数:
            table_name: 表名
            item_dict: 要插入或更新的数据字典
            unique_fields: 唯一约束字段列表
            spider: 当前运行的爬虫实例
        ...
        """
        # 检查记录是否存在
        existing_record = self._find_existing_record(table_name, item_dict, unique_fields)
        
        try:
            if existing_record:
                # 记录存在，执行更新
                self._update_record(table_name, item_dict, unique_fields)
                self.conn.commit()
                spider.logger.debug(f"已更新记录: {table_name} {unique_fields}")
                return False
            else:
                # 记录不存在，执行插入
                self._insert_record(table_name, item_dict)
                self.conn.commit()
                spider.logger.debug(f"已创建新记录: {table_name} {unique_fields}")
                return True
        except Exception as e:
            self.conn.rollback()
            spider.logger.error(f"update_or_create操作失败: {str(e)}")
            raise
    
    def _find_existing_record(self, table_name, item_dict, unique_fields):
        """
        根据唯一约束字段查找现有记录
        """
        # 构建WHERE条件
        ...
        for field in unique_fields:
            if field in item_dict:
                ...
                params[field] = item_dict[field]
            else:
                # 如果唯一约束字段在数据中不存在，则认为记录不存在
                return None
    ...
    def _update_record(self, table_name, item_dict, unique_fields):
        """
        更新现有记录
        """
        # 构建SET子句
        ...
        for field, value in item_dict.items():
            if field not in unique_fields:  # 不更新唯一约束字段
                set_clauses.append(f"{field} = %({field}_update)s")
                params[f"{field}_update"] = value
        ...
        # 构建WHERE子句
        where_clauses = [f"{field} = %({field})s" for field in unique_fields]
        where_sql = " AND ".join(where_clauses)
    ...
    def _insert_record(self, table_name, item_dict):
        """
        插入新记录
        """
        fields = item_dict.keys()
        field_names = ", ".join(fields)
        placeholders = ", ".join([f"%({k})s" for k in fields])
        
        # 执行插入
        query = f"INSERT INTO {table_name} ({field_names}) VALUES ({placeholders})"
        self.cur.execute(query, item_dict)


# 示例：如何在Scrapy爬虫中使用UpdateOrCreatePostgreSQLPipeline
"""
# 方法1：在settings.py中配置Pipeline，自动处理yield item
# 这是最简单的使用方式，Scrapy会自动为你调用process_item方法
ITEM_PIPELINES = {
    # 使用update_or_create功能的Pipeline
    'sf_spider.pipelines.UpdateOrCreatePostgreSQLPipeline': 300,
}

# 在爬虫中只需简单yield item即可，无需手动调用任何方法
class SimpleExampleSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # 解析数据并创建Item
        item = ExampleItem()
        item['tracking_number'] = response.xpath('//div[@class="tracking-number"]/text()').get()
        item['status'] = response.xpath('//div[@class="status"]/text()').get()
        item['update_date'] = datetime.now()
        
        # 只需yield item，Scrapy会自动调用配置的Pipeline处理
        # UpdateOrCreatePostgreSQLPipeline会根据唯一约束自动执行更新或创建
        yield item

# 方法2：在爬虫中手动使用（高级用法）
class ManualExampleSpider(scrapy.Spider):
    name = "manual_example"
    
    def __init__(self, *args, **kwargs):
        super(ManualExampleSpider, self).__init__(*args, **kwargs)
        # 创建UpdateOrCreatePostgreSQLPipeline实例
        self.update_or_create_pipeline = UpdateOrCreatePostgreSQLPipeline()
    
    def start_requests(self):
        # 打开Pipeline的数据库连接
        self.update_or_create_pipeline.open_spider(self)
        yield scrapy.Request(url='https://example.com', callback=self.parse)
    
    def parse(self, response):
        # 解析数据并创建Item
        item = ExampleItem()
        item['unique_id'] = response.xpath('//div[@class="unique-id"]/text()').get()
        item['data_field'] = response.xpath('//div[@class="data-field"]/text()').get()
        item['update_time'] = datetime.now()
        
        # 手动调用update_or_create_item方法处理item
        try:
            created, processed_item = self.update_or_create_pipeline.update_or_create_item(item, self)
            if created:
                self.logger.info(f"新记录已创建: {item['unique_id']}")
            else:
                self.logger.info(f"现有记录已更新: {item['unique_id']}")
        except Exception as e:
    ...
    def closed(self, reason):
        # 关闭数据库连接
        self.update_or_create_pipeline.close_spider(self)
...
class CustomPipeline:
    ...
    def open_spider(self, spider):
        self.update_or_create_pipeline.open_spider(spider)
        
    def process_item(self, item, spider):
        # 只对特定类型的Item使用update_or_create
        if isinstance(item, ShipmentItem):
            created, processed_item = self.update_or_create_pipeline.update_or_create_item(item, spider)
            spider.logger.info(f"Shipment {'创建' if created else '更新'}: {item.get('tracking_number')}")
        return item
        
    def close_spider(self, spider):
        self.update_or_create_pipeline.close_spider(spider)
"""
